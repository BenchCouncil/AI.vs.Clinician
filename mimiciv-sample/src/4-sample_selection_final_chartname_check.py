...
df_charts1 = pd.read_csv(TO_ROOT + 'preprocess/mimiciv_ill_redio_subject_doctor_chart_lab.csv',
    usecols=[SUBJECT_ID,HADM_ID,ITEMID, CHARTTIME, VALUE,VALUENUM, VALUEUOM,LABEL])
df_charts2 = pd.read_csv(TO_ROOT + 'preprocess/mimiciv_not_redio_subject_doctor_chart_lab.csv',
    usecols=[SUBJECT_ID,HADM_ID,ITEMID, CHARTTIME, VALUE,VALUENUM, VALUEUOM,LABEL])
df_charts = pd.concat([df_charts1,df_charts2])
...
df_prescription = pd.read_csv(ROOT+'prescriptions.csv',usecols=[SUBJECT_ID,HADM_ID,STARTTIME,DRUG])
...
def get_cardiovascular_data(df_charts_illtime_before,end_time):
    chart_name_list = ['Dopamine','Dobutamine','Epinephrine']
    df_chart_12h = get_start_to_endtime(df_charts_illtime_before,end_time - pd.Timedelta(hours=12), end_time + pd.Timedelta(hours=12))
    df_chart = df_chart_12h[df_chart_12h[[LABEL]].apply( lambda x: x.str.contains('|'.join(chart_name_list), case=False)).any(axis=1)]

    def getlabel(label):
        value = None
        df_chart_result = df_chart[df_chart[LABEL] == label]
        ...
        if len(df_chart_result) > 0:
            value = df_chart_result.iloc[0][VALUE]
        return value
    dopamine = getlabel('Dopamine')
    dobutamine = getlabel('Dobutamine')
    epinephrine = getlabel('Epinephrine')
    norepinephrine = getlabel('Norepinephrine')
    return dopamine,dobutamine,epinephrine,norepinephrine

def get_urine(df_charts,endtime):
    #看到官网中计算的 sofa的尿量检查项是这个
    df_charts = df_charts[df_charts[CHARTTIME] <= endtime]
    urineoutput_df_chart = df_charts[df_charts[ITEMID].isin([227488])]
    urineoutput = None
    if len(urineoutput_df_chart) > 0:
        urineoutput_df_chart[VALUE] = urineoutput_df_chart[VALUE].apply(lambda x: -x if x > 0 else x)
        urineoutput = sum(urineoutput_df_chart[VALUE])
    return urineoutput
...
def get_next_current(subject_id, hadm_id, start_time, end_time, blood_routine_dict, pathogenic_blood_dict, radio_dict,
    ...
    if len(next_current_list) == 0:
        ...
        print(hadm_id)
        print(start_time)
        print(blood_routine_dict)
        print(hemostasis_dict)
        print(pathogenic_blood_dict)
        print(culture_dict)
        print(smear_dict)
...
def to_sample_result_csv():
    df_subject1 = pd.read_csv(TO_ROOT + 'preprocess/1-mimiciv_ill_redio_subject_data_distribute_havecxr.csv',encoding='gbk')
    df_subject2 = pd.read_csv(TO_ROOT + 'preprocess/1-mimiciv_not_redio_subject_data_distribute_havecxr.csv',encoding='gbk')
    df_subject = pd.concat([df_subject1,df_subject2])
...
def process_sample_result(args):
    ...
    # df_radio_report_illtime_before = get_endtime_before(df_radio_report, subject_id, hadm_id, end_time + timedelta(hours=15))
    ...
    # demographic_info = get_demographic_info(demographic_dict,get_endtime_before(df_charts, subject_id, hadm_id, end_time))
    ...
    if current_list is not None and next_current_list is not None:
        if len(current_list) >=5 and len(next_current_list) >=3 :
            to_csv(subject_id, hadm_id, ill_time, start_endtime,start_time,end_time)


def to_csv(subject_id, hadm_id, ill_time, start_endtime,start_time,end_time):
    ...
    filename = TO_ROOT + 'preprocess/2-mimiciv_ill_and_not_have_cxr_5_4_check.csv'
...
def induce_dict(dict, key):
    if dict is not None:
        ...
        if key == '影像报告':
            ...
            value_list = value_list[:24]
            dict = {key: value_list}
    return dict

if __name__ == '__main__':
    print('Reconfirming data richness.')
