...
df_charts = pd.read_csv(TO_ROOT + 'preprocess/mimiciv_not_redio_subject_doctor_chart_lab.csv',
...
def select_period_csv():
    df_subject = pd.read_csv(
        TO_ROOT + 'preprocess/mimiciv_not_redio_subject.csv',
        usecols=[SUBJECT_ID, HADM_ID, '性别', '年龄', ADMITTIME], encoding='gbk')
    print('正在多线程计算中--------------  ')
...
def process_row(args):
    ...
    # random_time = np.random.normal(88, 149, 1)
    # ill_time = pd.to_datetime(row[ADMITTIME]) + pd.Timedelta(hours=random_time[0])
    ill_time = admittime

    #这里的时间范围不会影响补充数据饿有效期时间
    df_charts_illtime_before = get_endtime_before(df_charts, subject_id, hadm_id, ill_time + timedelta(hours=336))
    df_input_illtime_before = get_endtime_before(df_input, subject_id, hadm_id, ill_time + timedelta(hours=336))
    df_output_illtime_before = get_endtime_before(df_output, subject_id, hadm_id, ill_time + timedelta(hours=336))
    df_microbiology_illtime_before = get_endtime_before(df_microbiology, subject_id, hadm_id, ill_time + timedelta(hours=336))
    # df_radio_report_illtime_before = get_endtime_before(df_radio_report, subject_id, hadm_id,
    #                                                     ill_time + timedelta(hours=15))
    df_procedure_illtime_before = get_endtime_before(df_procedure, subject_id, hadm_id, ill_time + timedelta(hours=336))
    ...
    pathogenic_blood_dict = get_pathogenic_blood(df_charts_illtime_before, df_microbiology_illtime_before,df_subject_diagnose, ill_time + timedelta(hours=336))
    ...
    smear_dict = get_smear(df_charts_illtime_before, df_subject_diagnose, ill_time + timedelta(hours=336))

    df = df_subject.loc[[index]].copy()
    df['体重'] = get_weight(df_charts_illtime_before)

    print(f'subject_id:{subject_id},hadm_id:{hadm_id},正在统计数据分布')
    get_sample(df,'', admittime + pd.Timedelta(hours=4), admittime + pd.Timedelta(hours=336),subject_id, hadm_id, temperature_dict, blood_pressure_dict,
...
result_filname = TO_ROOT + 'preprocess/1-mimiciv_not_redio_subject_data_distribute.csv'

def get_sample(df,time_period,starttime_current, endtime_current, subject_id, hadm_id, temperature_dict, blood_pressure_dict,
            heart_rate_dict, input_dict, output_dict, respiratory_rate_dict, consciousness_dict,
            blood_pressure_systolic_dict, blood_routine_dict, pathogenic_blood_dict,
            radio_dict, gas_analysis_dict, hemostasis_dict, culture_dict, smear_dict):
...
if __name__ == '__main__':
    print('Filtering for data-rich non-septic patients.')
    select_period_csv()
    print("end")
