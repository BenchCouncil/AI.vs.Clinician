...
def select_period_csv():
    df_subject = pd.read_csv(
        TO_ROOT + 'preprocess/mimiciv_not_redio_subject.csv', encoding='gbk')
    print('正在多线程计算中--------------  ')
...
def process_row(args):
    ...
    age = row['年龄']
    # weight = row['体重']
    # temp_time = pd.to_datetime(row['start_endtime'].split('~')[0])
    ...
    while temp_time <= admittime + pd.Timedelta(hours=336):
        ...
        if result :
            ...
            to_csv_doctor(subject_id,hadm_id,start_endtime,gender,age,weight,temperature,blood_pressure,heart_rate,
                          ...
                          lymphocytes,hemoglobin,crp,pO2,o2,ph,hco3,inr,fib,ptt,plat,bilirubin,map,creatinine,pct, dopamine, dobutamine,
                        epinephrine, norepinephrine,lactate)
...
def to_csv_doctor(subject_id,hadm_id,start_endtime,gender,age,weight,temperature,blood_pressure,heart_rate,qsofa_rr,qsofa_sbp,qsofa_gcs,qsofa_score,output_sum,
    ...
    filename = TO_ROOT + 'preprocess/8-mimiciv_not_sample_fill_doctor_data_table.csv'
