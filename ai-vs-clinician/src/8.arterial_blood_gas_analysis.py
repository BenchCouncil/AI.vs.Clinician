...
def extract_unit(value_unit):
    unit_list = ['l/min','mmol/L','mm hg','units','mmol/l','mmhg','mg/l','%']
    for u in unit_list:
...
if __name__ == '__main__':
    ...
    for index, row in df_sample.iterrows():
        ...
        for next_check in next_check_list:
            if list(next_check.keys())[0] == '动脉血气分析':
                value_list_dict = next_check.get('动脉血气分析')
        ...
        check_his_dict = row['下一步检查_动脉血气分析（历史）']
        if str(check_his_dict) != 'nan':
            ...
            for value_dict in value_lists:
                ...
                for value in value_list:
                    ...
                    row_data.append(HIS)
                    df.loc[len(df)] = row_data
    df.to_csv(root + '\\datasets\\Arterial_Blood_Gas_Analysis.csv', encoding='gbk', index=False)
