...
column_list = ['subject_id', 'hadm_id','admittime','illtime','starttime','endtime', 'chronic airway obstruction',
...
result_csv = TO_ROOT + 'tscore_mimiciii_ill_model_input_data_by_diff_meanvalue.csv'
...
def process_ill_row(args):
    ...
    while i < 24:
        i = i + 1
        if i == 1:
            start_time = ill_time - timedelta(hours=i)
            end_time = ill_time
            period_end_to_illtime = 0.01
        else:
            start_time = ill_time - timedelta(hours=i)
            end_time = start_time + timedelta(hours=1)
            period_end_to_illtime = round(((ill_time - end_time).total_seconds() / 3600), 2)
        ...
        get_subject_charts(subject_id, hadm_id, df_chart_current_period, df_subject_diagnose_set, flag, df_12h, df_2h,
                           gender, start_time, end_time, admit_time,ill_time, ill_label,period_end_to_illtime)
...
def get_subject_charts(subject_id, hadm_id, df_chart_current_period, df_subject_diagnose_set, ae_flag, df_12h, df_2h,
                       gender, start_time, end_time, admit_time,ill_time,ill_label, hour_to_illtime):
    # 在当前检查项的时间段中筛选每列的值
    ...
    paco2 = get_final_chart_value(['Arterial PaCO2'], df_chart_current_period,  ae_flag, df_12h, start_time,
                                  end_time, hour_to_illtime,'arterial PaCO2')
    temperature = get_final_chart_value(['Temperature '], df_chart_current_period,  ae_flag, df_2h,
                                        start_time, end_time, hour_to_illtime,'temperature')
    weight = get_final_chart_value(
        ['Daily Weight', 'Admission Weight', 'Previous Weight', 'Present Weight', 'Weight Kg'], df_chart_current_period,
         ae_flag, df_12h, start_time, end_time, hour_to_illtime,'weight')
    ...
    row_list = [subject_id, hadm_id,str(admit_time),str(ill_time),str(start_time), str(end_time),
                ...
                sodium, sbp, temperature, wbc,ill_label,hour_to_illtime]
...
def get_final_chart_value(chart_name_list, df_chart_current_period, ae_flag, df_xxh, start_time, end_time,
                          hour_to_illtime,chart_column_name):
