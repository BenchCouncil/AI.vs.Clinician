...
TO_ROOT = f'{PATH}/hai-med-database/mimic-coxphm/data/mimiciv_preprocess/'
...
df_tscore_charts = pd.read_csv(TO_ROOT + 'lstm_mimiciv_train_subject_tscore_charts.csv')
...
result_csv = TO_ROOT + 'tscore_mimiciv_ill_model_input_data_by_diff_meanvalue.csv'

# 准备患病患者 和 正常患者
df_sample_subject = pd.read_csv(TO_ROOT + '13-mimiciv_train_5_4_from_admittime.csv',encoding='gbk')
...
#筛选患病患者
df_sample_subject = df_sample_subject[~df_sample_subject['ILL_TIME'].isna()]
...
def process_not_row(args):
    ...
    while i < 24:
        ...
        get_subject_charts(subject_id, hadm_id, df_chart_current_period, df_subject_diagnose_set, flag, df_12h, df_2h,
                           gender, start_time, end_time, admit_time, ill_time,ill_label,'',period_end_to_illtime,death_time)

        #患病患者用period_end_to_illtime补充  非患病患者用hour_to_admittime补充
...
def get_subject_charts(subject_id, hadm_id, df_chart_current_period, df_subject_diagnose_set, ae_flag, df_12h, df_2h,
                       gender, start_time, end_time, admit_time,ill_time,ill_label,hour_to_admittime,hour_to_illtime,death_time):
    # 在当前检查项的时间段中筛选每列的值
