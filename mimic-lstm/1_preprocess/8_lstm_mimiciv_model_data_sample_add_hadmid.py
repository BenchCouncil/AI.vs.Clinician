...
def sample_current_perdiod_label():
    ...
    #这两个字段表示sample的当前时间段 不是动态滑动窗口的时间段  是用来做预测时候可以对应的
    df_sample_label['start_time'] = df_sample_label['current_period'].str.split('~').str[0]
    df_sample_label['end_time'] = df_sample_label['current_period'].str.split('~').str[1]
...
def sample_test(df):
    ...
    process_model_input(df_test_input,ROOT + f'diff_meanvalue_del_missdata_0.4/lstm_mimiciv_sample_model_input_data_by_{kind}_predict.csv')
