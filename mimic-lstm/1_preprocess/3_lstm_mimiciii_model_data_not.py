...
result_csv = TO_ROOT + 'lstm_not_model_input_data_by_diff_meanvalue.csv'
...
def process_not_row(args):
    ...
    random_value = np.random.normal(84, 142)
    while random_value < 0:
        random_value = np.random.normal(84, 142)

    while temptime < admit_time + pd.Timedelta(hours=int(random_value)):
        start_time = temptime
        ...
        get_subject_charts(subject_id, hadm_id, df_chart_current_period, df_subject_diagnose_set, flag, df_12h, df_2h,
                           gender, start_time, end_time, admit_time, hour_to_admintime,death_time)
...
def get_subject_charts(subject_id, hadm_id, df_chart_current_period, df_subject_diagnose_set, ae_flag, df_12h, df_2h,
                       gender, start_time, end_time, admit_time,hour_to_admittime,death_time):
    # 在当前检查项的时间段中筛选每列的值
