...
def extract_unit(value_unit):
    unit_list = ['%',' 10^9/L','mmol/L','umol/L','g/L','fl','mg/l','pg','#/hpf','#/ul']
    for u in unit_list:
...
if __name__ == '__main__':
    ...
    for index, row in df_sample.iterrows():
        ...
        for next_check in next_check_list:
            if list(next_check.keys())[0] == '血常规':
                value_list_dict = next_check.get('血常规')
        ...
        check_his_dict = row['下一步检查_血常规（历史）']
    ...
    df.to_csv(root + '\\datasets\\Complete_Blood_Count.csv', encoding='gbk', index=False)
